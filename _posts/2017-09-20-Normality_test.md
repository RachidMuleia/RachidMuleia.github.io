...
Figure 1 shows the power comparison under the aforementioned distributions. Under the gamma distribution on the left panel one can see that, although the power does not increase slowly with sample size, Shapiro Wilk outperforms all test then is followed by Shapiro-Francia and Anderson-Darling. The same pattern is observed on the plot on the right-hand side. On both plots, it is also apparent the Kolmogorov-Smirnov poorly performs. Additionally, it can be observed that for small sample sizes almost all the tests show a lower power, pointing out that for small sample size the tests should be used with caution.
